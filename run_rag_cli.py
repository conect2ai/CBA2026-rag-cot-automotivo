...
python run_rag_cli.py --model deepseek-ai/DeepSeek-R1-Distill-Qwen-7B --input perguntas_respostas_Polo_2025.json --output rag_cli_output_deepseek-ai_DeepSeek-R1-Distill-Qwen-7B_20260311_104139.json --brand Volkswagen --car_model Polo --year 2025
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Nome ou caminho do modelo generativo a usar. Por padrão, usa o modelo definido em rag_hf_optimized.",
    )
    parser.add_argument(
        "--input",
        type=str,
...
def main() -> None:
    ...
    if args.output == "results.json":
        auto_name = build_filename_from_model(args.model, prefix="rag_cli_output")
    ...
    for idx, item in enumerate(data, 1):
        ...
        if question in processed_questions:
            continue
        print(f"[{idx}/{len(data)}] Processando pergunta: {question}")
        ...
        gen_original = generation_answer(
            prompt_original,
            model=model,
            tokenizer=tokenizer,
        )
        # Gera outra resposta usando uma versão embaralhada do mesmo contexto.
        retrieved_shuffled = retrieved[:]
        random.shuffle(retrieved_shuffled)
        shuffled_texts = [r["text"] for r in retrieved_shuffled]
        prompt_shuffled = build_prompt(question, shuffled_texts)
        gen_shuffled = generation_answer(
            prompt_shuffled,
            model=model,
            tokenizer=tokenizer,
        )
